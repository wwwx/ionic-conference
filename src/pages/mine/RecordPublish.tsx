...
const Conference: React.FC = () => {
  ...
  const hasNotStartData = list.filter(
    (item: RecordParams) => item.type === RecordType.HAS_NOT_START
  );

  const processingData = list.filter(
    (item: RecordParams) => item.type === RecordType.PROCESSING
  );
    ...
    >
      ...
      <TabPanel>
        {processingData.map((item: RecordParams, i: number) => {
          // const typeName = item.isHost ? '主持' : '参与';
          return (
            <RecordProcessingCard data={item} key={i} onConfirm={handConfirm} />
          );
        })}
      </TabPanel>
    </Tabs>
